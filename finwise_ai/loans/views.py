...
def loan_home(request):
    ...
    total_debt = active_loans.aggregate(total=models.Sum('remaining_balance'))['total'] or 0
    
    # Calculate monthly payments
    monthly_payments = active_loans.aggregate(total=models.Sum('monthly_payment'))['total'] or 0
...
def loan_simulator(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('loan_simulation_result', loan_id=loan.pk, eligibility_id=eligibility.pk)
    else:
...
def loan_payment_mark_paid(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('loan_detail', pk=payment.loan.pk)
